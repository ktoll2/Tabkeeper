...
public sealed class PinnedTabStateStoreTests : IDisposable
{
    ...
    [Fact]
    public void HasState_DistinguishesSavedBranchesFromUnknownOnes()
    {
        string statePath = Path.Combine(temporaryDirectory, "state.json");
        string repositoryRoot = Path.Combine(temporaryDirectory, "repository");
        PinnedTabStateStore stateStore = new PinnedTabStateStore(statePath);
        ...
        stateStore.SavePinnedPaths(repositoryRoot, "feature/empty", Array.Empty<string>());

        Assert.True(stateStore.HasState(repositoryRoot, "main"));
        Assert.True(stateStore.HasState(repositoryRoot, "feature/empty"));
        Assert.False(stateStore.HasState(repositoryRoot, "feature/never-seen"));
        Assert.False(stateStore.HasState(Path.Combine(temporaryDirectory, "other-repo"), "main"));
    }

    [Fact]
    public void SavePinnedPaths_PreservesOtherBranchesWhenTheFileAlreadyExists()
    {
        string statePath = Path.Combine(temporaryDirectory, "state.json");
        string repositoryRoot = Path.Combine(temporaryDirectory, "repository");
        PinnedTabStateStore stateStore = new PinnedTabStateStore(statePath);
        ...
        stateStore.SavePinnedPaths(repositoryRoot, "main", new[] { "Program.cs" });
        // A second store instance simulates another Visual Studio process writing the shared file.
        new PinnedTabStateStore(statePath).SavePinnedPaths(repositoryRoot, "feature/login", new[] { "Login.cs" });

        Assert.Equal(new[] { "Program.cs" }, stateStore.GetPinnedPaths(repositoryRoot, "main"));
        Assert.Equal(new[] { "Login.cs" }, stateStore.GetPinnedPaths(repositoryRoot, "feature/login"));
    }
