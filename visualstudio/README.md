...
Prerequisites: .NET SDK 10.0 or later, and Visual Studio 2022 or 2026 with the Visual Studio
extension development workload (for packaging and debugging the VSIX on Windows).
...
`make test` restores, builds, and runs the test suite in one step. Other targets include `make
build`, `make package`, `make current-version` / `make stamp-version VERSION=1.2.3`, and `make
clean`.
...
The built `.vsix` lands under `Tabkeeper.Vsix/bin/Release/` and is intentionally ignored by Git.
